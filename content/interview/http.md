...
{{% admonition type="info" title="CSRF：跨站请求伪造" details="true" %}}
...
- 种类
    - 反射性：通常诱使用户 点击一个恶意链接，或者 提交一个表单, 在用户点击链接或提交表单的同时向用户访问的网站注入脚本。
    - 存储型：例如发表评论，评论里带有恶意脚本，这样被存储入库后，后续的用户浏览该评论时都会执行恶意脚本
    - 基于DOM：用户的页面上可能会获取显示url上的参数，此时修改url传参为恶意代码，这样会直接被插入到DOM结构。
...
> CSRF是利用网站对用户的信任。XSS是利用用户对指定网站的信任；
